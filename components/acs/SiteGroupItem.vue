<template>
  ...
  >
    <div class="h-10 w-10 shrink-0 rounded-full bg-gradient-to-br from-slate-100 to-slate-300 p-1.5">
      <img src="/icon/global.png" alt="" class="h-auto w-full" />
    </div>
    <div class="flex grow flex-col">
      <span class="text-xs font-semibold text-gray-700">{{ group.name }}</span>
      <span class="text-xs font-normal text-gray-500">{{ group.count }} sites</span>
    </div>
    <div
...
export default {
  ...
  methods: {
    onClick() {
      this.$router.push({ path: '/access/site-group/save', query: { groupId: this.group.id } })
    },
    onDragStart(e) {
      if (!this.getPermissionStatus('drag a site group')) return
      this.$store.commit('acs/RESET_GROUP_ATTACH_PARAMS')
      this.$store.commit('acs/UPDATE_GROUP_ATTACH_PARAMS', {
        siteGroupType: 'group',
        siteGroupId: this.group.id,
      })
    },
    onDrop(e) {
      if (!this.getPermissionStatus('drop on a site group')) return
      this.$store.commit('acs/UPDATE_GROUP_ATTACH_PARAMS', {
        siteGroupType: 'group',
        siteGroupId: this.group.id,
      })
