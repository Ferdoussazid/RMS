<template>
  <div class="col-span-12 min-h-[60vh] w-full rounded bg-slate-100 lg:col-span-5">
    ...
    <div v-else class="flex flex-col gap-2 overflow-hidden">
      <div v-if="!sarbsAlarmItems.length" class="flex flex-col items-center col-span-12 m-auto space-y-6">
        <img src="/icon/empty.png" alt="" class="w-16 h-auto" />
        <span class="text-sm font-light text-gray-500">No Alarms Found</span>
      </div>
      <div v-if="sarbsAlarmItems.length" class="flex flex-col items-center justify-start col-span-12">
        <div class="flex items-center justify-center pt-4">
          <span class="text-sm font-medium text-gray-700 select-none">RECTIFIER ALARMS</span>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      sarbsAlarmItems() {
        const sarbsRectifierAlarmGroup = this.rectifierAlarmGroups.find(
          (g) => g.name === OEM_ALARM_GROUP.SARBS_ALARM
        );
        ...
        if (sarbsRectifierAlarmGroup) {
          ...
          const otherAlarms = sarbsRectifierAlarmGroup.items.filter(
            (alarm) => !this.priorityIds.includes(alarm.id)
          );
