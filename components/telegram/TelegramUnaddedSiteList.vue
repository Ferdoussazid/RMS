<template>
  <div class="flex flex-1 flex-col space-y-2 rounded transition duration-300 ease-in-out">
    ...
    <div class="relative max-h-[200px] min-h-[200px] overflow-scroll rounded shadow-md scrollbar-hide md:max-h-[290px]">
      <div v-for="(site, i) in filteredSites" :key="site.uid" :class="[i % 2 === 0 ? 'bg-slate-100' : 'bg-white']"
  ...
  computed: {
    ...
    areaList() {
      if (this.companyName === COMPANY.SUMMIT_COM) {
        const items = this.items.map(s => this._.get(s, 'place.zone', '')).filter(a => !!a)
        return this._.uniq(items)
      } else {
        const items = this.items.map(s => this._.get(s, 'place.division', '')).filter(a => !!a)
        return this._.uniq(items)
      }
    },
    clusterSelectLabel() {
    ...
    clusterList() {
      ...
      const items = this.items
        .filter(s => _.get(s, 'place.division', '') === this.area || this.companyName === COMPANY.SUMMIT_COM)
        .map(s => this.companyName === COMPANY.SUMMIT_COM ? _.get(s, 'place.cluster', '') : _.get(s, 'place.district', ''))
        .filter(c => !!c)
      return _.uniq(items)
    },
    filteredSites() {
      ...
      if (this.cluster && this.companyName === COMPANY.SUMMIT_COM) {
        items = items.filter(s => this._.get(s, 'place.cluster', '') === this.area)
      }
