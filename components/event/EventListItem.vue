      ...
      <div class="flex grow flex-col gap-1.5 pl-1.5">
        <div class="flex grow flex-col gap-0.5">
          <div v-if="showSiteName" class="flex flex-row items-center justify-start space-x-1 text-[0.6rem]">
            <span class="cursor-pointer font-semibold text-indigo-500" @click="onSiteNameClick">{{ siteName }}</span>
          </div>
...
import { getEventColor } from '~/assets/js/EventStyle'
...
export default {
  ...
  computed: {
    ...
    colorbar() {
      return getEventColor(this.event.name)
    },
  },
  methods: {
    getSiteFromEvent() {
    ...
    getRedirectionPath() {
      return RMS_EVENTS.includes(this.event.name) ? '/site' : '/access'
    },
    setFocusedTenant() {
