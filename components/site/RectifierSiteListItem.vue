<template>
  <div
    class="grid items-stretch justify-center grid-cols-12 py-4 transition duration-300 bg-white border-b border-slate-200 md:border-0 md:py-0"
    :class="[theme]">
    ...
    <div
      class="col-span-6 py-4 pl-4 pr-2 space-y-1 text-center break-words break-all border-0 border-gray-200 md:col-span-1 md:border-l lg:text-left">
      <span class="text-xs font-medium text-gray-700 capitalize">{{ tenantLabel }}</span>
    </div>
    <div
      class="col-span-6 py-4 pl-4 pr-2 space-y-1 text-center break-words break-all border-0 border-gray-200 md:col-span-1 md:border-x lg:text-left">
      <span class="text-xs font-medium text-gray-700 capitalize">{{ MNOSiteID }}</span>
    </div>
    <div class="grid grid-cols-12 col-span-12 place-items-center md:col-span-8 md:flex">
      <div v-if="!isReadOnlyUser" class="flex items-start justify-center w-full h-full col-span-6 px-1 py-3 md:flex-1">
        <SimpleIconStatus :type="deviceAlarm" :with-device="siteWithDevice" label="Device" ic-success="fa-signal"
          ic-error="fa-ban" />
      </div>

      <div class="flex items-start justify-center h-full col-span-6 px-1 py-3 md:flex-1">
        <SimpleIconStatus :type="acFailAlarm" label="AC Fail" />
      </div>
      <div class="flex items-start justify-center h-full col-span-6 px-1 py-3 md:flex-1">
        <SimpleIconStatus :type="dcLowAlarm" label="DC Low" />
      </div>
      <div v-if="!isReadOnlyUser" class="flex items-start justify-center h-full col-span-6 px-1 py-3 md:flex-1">
        <SimpleIconStatus :type="rectifierFailAlarm" label="Rectifier Fail" />
      </div>
      <div
        class="flex items-start justify-center h-full col-span-6 px-1 py-3 border-0 border-gray-200 md:flex-1 md:border-l">
      ...
      <div v-if="!isReadOnlyUser"
        class="flex items-start justify-center h-full col-span-6 px-1 py-3 border-0 border-gray-200 md:flex-1 md:border-l">
        <SimpleIconStatus :type="doorAlarm" label="Door" flat ic-success="fa-door-closed" ic-error="fa-door-open"
          ic-unavailable="fa-door-closed" size="lg" />
      </div>
      <div v-if="!isReadOnlyUser" class="flex items-start justify-center h-full col-span-6 px-1 py-3 md:flex-1">
        <SimpleIconStatus :type="lockAlarm" label="Lock" flat ic-success="fa-lock" ic-error="fa-lock-open"
          ic-unavailable="fa-lock" size="lg" />
      </div>

      <div v-if="!isReadOnlyUser" class="flex items-start justify-center h-full col-span-6 px-1 py-3 md:flex-1">
        <SimpleIconStatus :type="securityBreachAlarm" label="Security Breach" />
      </div>
      <div v-if="isFTB && !isReadOnlyUser"
        class="flex flex-col items-center justify-center h-full col-span-6 px-1 py-3 border-0 border-gray-200 md:flex-1 md:flex-row md:items-start md:border-l">
        <span class="text-xs font-semibold text-blue-900">{{ terminalVoltage }}</span>
        <span class="block text-xs font-normal text-center text-gray-500 md:hidden">Terminal Voltage</span>
      </div>
      <div v-if="isFTB && !isReadOnlyUser"
        class="flex flex-col items-center justify-center h-full col-span-6 px-1 py-3 md:flex-1 md:flex-row md:items-start">
        <span class="text-xs font-semibold text-blue-900">{{ nonCriticalLoad }}</span>
        <span class="block text-xs font-normal text-center text-gray-500 md:hidden">Non Critical Load</span>
      </div>
      <div v-if="isFTB && !isReadOnlyUser"
        class="flex flex-col items-center justify-center h-full col-span-6 px-1 py-3 md:flex-1 md:flex-row md:items-start">
        <span class="text-xs font-semibold text-blue-900">{{ criticalLoad }}</span>
        <span class="block text-xs font-normal text-center text-gray-500 md:hidden">Critical Load</span>
      </div>

      <div
        class="flex items-center justify-center h-full col-span-6 py-3 border-0 border-gray-200 md:flex-1 md:border-l"
        @click="handleDropdownPosition">
        <DropdownButton :dropdown-position="dropdownMenuPosition" label="More" :options="allowedActions"
...
export default {
  ...
  computed: {
    ...
    focusedAlarmName() {
      return getLabelOfRectifierAlarm(this.withEvent)
    },
    zoneLabel() {
      const label = [
    ...
    eventName() {
      const label = getLabelOfRectifierAlarm(this.withEvent)
    ...
    eventTime() {
      const alarmTime = this.site.more_details.event_time
      if (!alarmTime) {
        return ''
      }
      const lastEventTime = new Date(alarmTime)
    ...
    tenantLabel() {
      return getTenantLabel(this.tenant, false)
    }
  },
