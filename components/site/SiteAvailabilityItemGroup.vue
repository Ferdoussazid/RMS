...
export default {
  ...
  watch: {
    filters(val, old) {
      this.setFilteredTenants()
    },
    dropdownTenant(val, old) {
      this.setFilteredTenants()
    },
  },
  mounted() {
  ...
  methods: {
    setFilteredTenants() {
      this.filteredTenants = getTenantsFromFilters(this.site, this.dropdownTenant, this.filters, this.allowedTenants)
    },
  },
}
</script>
