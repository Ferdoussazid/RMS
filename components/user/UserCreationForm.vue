<template>
  <div v-if="showModal"
    ...
    <div class="w-11/12 rounded bg-white shadow md:w-1/2 lg:w-1/3">
      <div
        class="flex shrink-0 flex-row items-center justify-between overflow-hidden rounded-t border-b border-gray-200 bg-teal-500 px-4 py-2">
        <div class="flex items-center gap-2 text-sm font-semibold text-white">
          <span><i class="fas fa-user"></i></span><span>Create New User</span>
        </div>
        <div>
      ...
      <div class="p-6">
        <div class="flex w-full flex-col space-y-3">
          <div v-if="isSudoUser" class="flex flex-col gap-1">
            <div>
          ...
          <div class="flex flex-col gap-1">
            <FixedLabelInput v-model="form.password" required autocomplete="off" input-type="password" label="Password"
              placeholder="" />
            <div v-if="errors.password" class="text-xs italic text-red-500">{{ errors.password }}</div>
          </div>
          <div class="flex flex-col gap-1">
            <FixedLabelInput v-model="form.confirmationPassword" required autocomplete="off" input-type="password"
              label="Confirm Password" placeholder="" />
            <div v-if="errors.confirmationPassword" class="text-xs italic text-red-500">
          ...
          <div class="flex flex-col gap-1">
            ...
            <div v-for="access in userAccessList" :key="access.id" class="gap-2 py-2 flex align-middle">
              <ToggleSwitch :status="access.checked" @toggleStatus="toggleStatus(access)" />
              <span class="text-sm font-medium text-gray-700">{{ access.name }}</span>
        ...
        <div class="mt-3 flex justify-end gap-2">
          ...
          <button
            class="flex cursor-pointer items-center justify-center gap-1 rounded bg-teal-500 px-3 py-1.5 shadow-lg hover:bg-teal-700"
            @click="submitForm">
            ...
            <span class="text-sm font-semibold text-white">Create</span>
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      errors: {},
      tenants: [
        { id: 1, name: 'Select Tenant', value: null },
      ...
      userAccessList: USER_ACCESS_LIST.filter(access => access.public).map((access, i) => ({
        id: i,
        name: access.name,
        value: access.value,
        checked: false,
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (this.form.password.trim().length < 6) {
        this.errors.password = 'Please enter a password with at least 6 characters'
      }
      if (!this.form.confirmationPassword) {
        this.errors.confirmationPassword = 'Please confirm the password'
      }
      ...
      if (this.form.password.trim() !== this.form.confirmationPassword.trim()) {
        this.errors.confirmationPassword = 'Passwords do not match'
      }
