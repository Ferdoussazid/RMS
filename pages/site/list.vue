<template>
  <div class="flex flex-col w-full">
    ...
    <div class="flex flex-col w-full h-full mt-2 bg-white rounded shadow-md">
      <SiteFilterForm :alarm-filters="filters" :focused-event-name="focusedEventName" @filter="handleFilteredSites"
        @tenant="handleDropdownTenant" @filter-click="handleFilterClick" />
      <div class="flex flex-col w-full divide-y divide-gray-100">
        <SiteListHeader v-show="!fetching && filteredSites.length" />
        <div ref="listContainer" class="flex h-[70vh] w-full flex-col items-stretch overflow-scroll">
          ...
          <div v-for="(site, j) in visibleSiteItems" :key="site.uid + j" class="divide-y">
            <SiteListItem v-for="(tenant, i) in getFilteredTenants(site)" :key="site.uid + i" :serial="j + 1"
              :site="site" :with-event="focusedEventName" :tenant="tenant" />
          </div>
